defmodule SimpleBudgetingWeb.Locations.Show do
  @moduledoc false
  use SimpleBudgetingWeb, :live_view

  import Ecto.Query, warn: false

  alias SimpleBudgeting.Schema.Location
  ...
  @impl true
  def handle_event("submit", %{"location" => params}, socket) do
    changeset =
      socket.assigns.location
      |> Location.changeset(params)
      |> Map.put(:action, :update)
  ...
  defp set_assigns(socket) do
    location =
      Location
      |> SimpleBudgeting.Repo.get(socket.assigns.location_id)
